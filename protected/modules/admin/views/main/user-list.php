...
<script type="text/javascript">//static
    ...
    function content_refresh(){
        ...
        //分页
        Pager($('#page'),'Main','UserList',{},params);

        //搜索
        $('#search').click(function(){
            var search = $('#search_type').val()+':'+$('#search_val').val();
            State.forward('Main','UserList',{search:search,p:params.page});
        });
        //排序
        $('[id^=order_]').click(function(){
            var val = this.id.replace('order_',''),
                ...
                order_str = val+':'+sc;
            State.forward('Main','UserList',{order:order_str,p:params.page});
        });
        //点击修改
        $('[id^=attr_]').attr('ContentEditable',true).focus(function(){
            ...
            if(val !== $(this).data('val')){
                oneAjax('Main','AjaxEditOne',{type:'User',id:id,attr:attr,val:val},function(o){
                    if(o.code!=1){
...
alert(o.errors[attr][0]);
        ...
        $('#delete').click(function(){
            ...
            ids = ids.join(',');
            oneAjax('Main','AjaxDelete',{type:'User',ids:ids},function(o){
                if(o.code==1){
                    State.back(0);
                }else{
                    alert('error');
