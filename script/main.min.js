...
var YCache=function(uniqueN){var uniqueN=(typeof(uniqueN)!="string")?"":"uniqueN_"+uniqueN+"_";setCookie=function(name,value){var Days=1;var exp=new Date();exp.setTime(exp.getTime()+Days*24*60*60*1000);document.cookie=name+"="+escape(this.encode(value))+";expires="+exp.toGMTString();}
getCookie=function(name){var arr=document.cookie.match(new RegExp("(^| )"+name+"=([^;]*)(;|$)"));if(arr!=null)
return this.unencode(unescape(arr[2]));return null;}
delCookie=function(name){var exp=new Date();exp.setTime(exp.getTime()-1);var tem=this.getCookie(name);if(tem!=null)
document.cookie="name="+tem+";expires="+exp.toGMTString();}
encode=function(str){var temstr="";var i=str.length-1;for(i;i>=0;i--){temstr+=str.charCodeAt(i);if(i)
temstr+="a";}
return temstr;}
unencode=function(str){var strarr="";var temstr="";strarr=str.split("a");var i=strarr.length-1;for(i;i>=0;i--){temstr+=String.fromCharCode(eval(strarr[i]));}
return temstr;}
return{set:function(text){setCookie(uniqueN,text);},clear:function(){delCookie(uniqueN);},get:function(){return getCookie(uniqueN);}}}
var State={};(function(State){var history=[],Dic={},Cache=YCache('siteHistory');State.setDic=function(p){if(typeof p!='object'){return false;}
for(var k in p){Dic[k]=p[k];}}
State.setDefaultPosition=function(url,params){loadFromCookie();if(history.length==0){history.push({url:url,params:params});saveToCookie();}}
State.forward=function(url,params){history.push({url:url,params:params});saveToCookie();State.gotoUrl(url,params);}
State.forwardNoback=function(url,params){State.gotoUrl(url,params);}
State.back=function(n){if(!n){n=history.length-1;}else if(n<0){n=(-n>history.length?history.length:-n)-1;}else{n=n>history.length?0:history.length-n;}
var h;for(var i=history.length-1;i;i--){h=history.pop();if(i==n){break;}}
saveToCookie();State.gotoUrl(h.url,h.params);}
State.gotoUrl=function(url,params){}
State.getPositionHtml=function(){var p={default:{},list:[]};for(var i=0;i<history.length;i++){if(i==0){p.default={url:getUrl(history[i].url,history[i].params),name:Dic[history[i].url]};}else{p.list.push({url:getUrl(history[i].url,history[i].params),name:Dic[history[i].url]});}}
return sitePositionTemplate.render(p);}
function saveToCookie(){var l=[];for(var i=0;i<history.length;i++){var pl=[];for(var j in history[i].params){pl.push(j+'**'+history[i].params[j]);}
l.push(history[i].url+'^^'+pl.join('&&'));}
var v=l.join('%%');Cache.set(v);}
function loadFromCookie(){var c=Cache.get();if(!c||c.indexOf('/')==-1){return;}
var hl=c.split('%%'),h=[];for(var i=0;i<hl.length;i++){var l=hl[i].split('^^'),p={};if(l[1]){var pl=l[1].split('&&');for(var ii=0;ii<pl.length;ii++){var pp=pl[ii].split('**');p[pp[0]]=pp[1];}}
h.push({url:l[0],params:p,});}
history=h;}})(State);function time(){return Math.floor(STIME+(new Date().getTime()-CTIME)/1000);}
function getUrl(c,a,p){if(a===undefined){var pieces=c.split("/");var arr=URLCACHE;for(var i=0;i<pieces.length;i++){if(arr[pieces[i]]){arr=arr[pieces[i]];}}
return BASEURL+"/"+c+"?v="+arr;}else{var url,param;if(typeof(a)=="string"){url=BASEURI+"/"+c+"/"+a;param=p;}else{url=BASEURI+"/"+c;param=a;}
if(param){var l=[];for(var k in param){l.push(encodeURIComponent(k)+"="+encodeURIComponent(param[k]));}
...
return ret;}
function yajax(c,a,data,succ_callback,dom){if(dom){var k_disable=0;$(dom).each(function(){if($(this).data("k_disable")){k_disable=1;}});if(k_disable){return;}else{$(dom).data("k_disable",1);}}
function show_error(error){alert(error?error:'data load error!');}
var succ=function(obj){dom&&$(dom).data("k_disable",0);if(obj==null||typeof(obj)!="object"||!obj.code||obj.code==-1){show_error(1);}else{succ_callback(obj);}}
